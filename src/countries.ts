...
export async function loadCountries(countriesFile: string, ...countries: string[]) {
  const countryMap = await loadCountryMap(countriesFile);
  return countries.map((country) => countryMap[country]);
}

export async function loadCountriesFrom(file: string): Promise<Array<Country>> {
  const countriesData: FeatureCollection = await loadJsonFile(file);
  return countriesData.features.map((feature) => ({ name: feature.properties?.ADMIN ?? "Unnamed", data: feature }));
}

export async function loadCountryMap(file: string): Promise<Record<string, Country>> {
  const countries = await loadCountriesFrom(file);
  return Object.fromEntries(countries.map((country) => [country.name, country]));
}

export const alaska: Country = {
  name: "Alaska",
  data: {
